...
async function startNewBrowser (browser) {
  if (browser) {   
    try {
      browser.close();
    } catch (err) {
  ...
  byNamesDuring(dates, newBrowser, page)
    .then(async (result) => {
      let value = result[0],
          err = result[1];
      ...
      console.log('\n#\n#\n#\n### "Un-managed" error. Let this go till log says "giving up". Or stop it yourself and deal with it a different way. 2\n#\n#\n#');
      console.log('status code:', err.statusCode)
      if (err.statusCode === 429) {
        do429(page, newBrowser);
      } else {
        waitThenRepeat(newBrowser);
      }
    });
...
let picCount = 0;
async function do429 (page, browser) {

  await page.screenshot({path: '429_' + picCount + '.png'});
  ...
  let html = await page.waitFor(
    function () {
      ...
      console.log('Is this html?', elem);
      return elem;
    }
  );
  // Should show how long we need to wait
  console.log('Is this html?', html);
};
